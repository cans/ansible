...
class HerokuCloudProvider(CloudProvider):
    ...
    def filter(self, targets, exclude):
        ...
        super(HerokuCloudProvider, self).filter(targets, exclude)

    def setup(self):
        """Setup the cloud resource before delegation and register a cleanup callback."""
        super(HerokuCloudProvider, self).setup()

        heroku_config_path = os.path.expanduser('~/.netrc')

        if os.path.exists(heroku_config_path) and not self.args.docker and not self.args.remote:
            raise ApplicationError('Rename "%s" or use the --docker or --remote option to isolate tests.' % heroku_config_path)
    ...
    def _setup_dynamic(self):
        ...
        if not self.args.explain:
            credentials = response['heroku']['credentials']
...
class HerokuCloudEnvironment(CloudEnvironment):
